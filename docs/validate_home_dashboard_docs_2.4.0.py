...
srs_p=base/'SRS_2.4.0.yaml'; sad_p=base/'SAD_2.4.0.yaml'
errs=[]; warns=[]
try:
    s=yaml.safe_load(srs_p.read_text()); d=yaml.safe_load(sad_p.read_text())
except Exception as e:
    print('errors=1\nPARSE',e); sys.exit(1)
if s['metadata']['version']!='2.4.0' or d['metadata']['version']!='2.4.0': errs.append('version mismatch')
...
if s['metadata']['companion_document']!='SAD_2.4.0.yaml' or d['metadata']['companion_document']!='SRS_2.4.0.yaml': errs.append('companion mismatch')
