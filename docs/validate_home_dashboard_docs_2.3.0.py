...
srs_p=base/'SRS_2.3.0.yaml'; sad_p=base/'SAD_2.3.0.yaml'
errs=[]; warns=[]
try: s=yaml.safe_load(srs_p.read_text()); d=yaml.safe_load(sad_p.read_text())
except Exception as e:
 print('errors=1\nPARSE',e); sys.exit(1)
if s['metadata']['version']!='2.3.0' or d['metadata']['version']!='2.3.0': errs.append('version mismatch')
if s['metadata']['status']!='draft-for-review' or d['metadata']['status']!='draft-for-review': errs.append('status not draft-for-review')
# requirements
reqs=[]
...
# 2.3 dimmer-management contracts
for rid in ['HUE-021','HUE-022']:
 if rid not in reqset: errs.append('2.3 requirement missing: '+rid)
for aid in ['AC-HUE-021','AC-HUE-022']:
 if aid not in acset: errs.append('2.3 acceptance missing: '+aid)
for text in ['HueAutomationGraph','DimmerProfile','DimmerMutationPlan','no automatic retry or rollback']:
 if text.lower() not in blob.lower(): errs.append('2.3 dimmer architecture missing: '+text)
# Backup privacy
