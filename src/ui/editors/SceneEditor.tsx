...
export function SceneEditor({ route, navigation }: { route?: any; navigation?: any }): JSX.Element {
  ...
  >
    ...
    <EditorSection title="Per-light scene state">
      ...
      {selectedLights.map((lightId) => {
        ...
        return <React.Fragment key={lightId}>
          ...
          <EditorCatalogNumberField fieldKey="bri" label={`Light ${lightId} brightness`} onChange={(bri) => setLightStates((current) => ({ ...current, [lightId]: { ...current[lightId], ...(bri === undefined ? {} : { bri }) } }))} testID={`scene-light-${lightId}-bri`} value={state.bri} />
          <EditorHueField label={`Light ${lightId} hue color`} onChange={(hue) => setLightStates((current) => ({ ...current, [lightId]: { ...current[lightId], ...(hue === undefined ? {} : { hue }) } }))} testID={`scene-light-${lightId}-hue`} value={state.hue} />
          <EditorCatalogNumberField fieldKey="sat" label={`Light ${lightId} saturation`} onChange={(sat) => setLightStates((current) => ({ ...current, [lightId]: { ...current[lightId], ...(sat === undefined ? {} : { sat }) } }))} testID={`scene-light-${lightId}-sat`} value={state.sat} />
          <EditorXyColorField label={`Light ${lightId} XY color`} onChangeText={(xy) => setLightStates((current) => ({ ...current, [lightId]: { ...current[lightId], ...(parsePair(xy) ? { xy: parsePair(xy) } : {}) } }))} testID={`scene-light-${lightId}-xy`} value={pairValue(state.xy)} />
          <EditorCatalogNumberField fieldKey="ct" label={`Light ${lightId} color temperature`} onChange={(ct) => setLightStates((current) => ({ ...current, [lightId]: { ...current[lightId], ...(ct === undefined ? {} : { ct }) } }))} testID={`scene-light-${lightId}-ct`} value={state.ct} />
          ...
          <EditorCatalogNumberField fieldKey="transitiontime" label={`Light ${lightId} transition duration`} onChange={(transitiontime) => setLightStates((current) => ({ ...current, [lightId]: { ...current[lightId], ...(transitiontime === undefined ? {} : { transitiontime }) } }))} testID={`scene-light-${lightId}-transitiontime`} value={state.transitiontime} />
    ...
    <EditorSection title="Scene details">
